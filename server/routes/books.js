...
router.post("/preview", async (req, res) => {
    ...
    try {
        // fetch book info from open library api using isbn
        ...
        if (olData.authors?.length > 0) {
            ...
            const authorResp = await fetch(`${baseUrl}${authorKey}.json`);
            if (authorResp.ok) {
        ...
        res.json({
            ...
            description: workData?.description?.value || "description not available",
...
router.delete("/:id", auth, async (req, res) => {
    try {
        const result = await sql`DELETE FROM books WHERE id = ${req.params.id} RETURNING *`;
